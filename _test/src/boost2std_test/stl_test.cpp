...
#if !__CCWRAP_NO_HEADER_ANY
#include <any>
#endif
#if !__CCWRAP_NO_HEADER_VARIANT
#include <variant>
#endif
#if !__CCWRAP_NO_HEADER_OPTIONAL
#include <optional>
#endif
#if !__CCWRAP_NO_HEADER_STRING_VIEW
#include <string_view>
#endif
...
CCWRAP_TEST(algorithm) {
    std::vector<int>    v;
    v.push_back(3);
    v.push_back(1);
    v.push_back(8);

    ccwrap_test( std::all_of(v.begin(), v.end(), less_n<10>()) );
    ccwrap_test( std::all_of(v.begin(), v.end(), less_n<5>()) == false);

    ccwrap_test( std::any_of(v.begin(), v.end(), less_n<5>()) );
    ccwrap_test( std::any_of(v.begin(), v.end(), less_n<0>()) == false );

    std::vector<int>    v2(3);
    std::copy_if(v.begin(), v.end(), v2.begin(), less_n<5>());
    ccwrap_test(v2[0] == 3 && v2[1] == 1 && v2[2] == 0);

    std::copy_n(v.begin(), 3, v2.begin());
    ccwrap_test(v2[0] == 3 && v2[1] == 1 && v2[2] == 8);

    ccwrap_test(std::find_if_not(v.begin(), v.end(), less_n<10>()) == v.end());
    ccwrap_test(std::find_if_not(v.begin(), v.end(), less_n<0>()) == v.begin());

    v2.clear();
    std::vector<int> v3;
    std::partition_copy(v.begin(), v.end(), std::back_inserter(v2), std::back_inserter(v3), less_n<5>());
    ccwrap_test(v2.size() == 2 && v2[0] == 3 && v2[1] == 1 && v3.size() == 1 && v3[0] == 8);

    std::vector<int>::iterator  it = std::partition_point(v.begin(), v.end(), less_n<5>());
    ccwrap_test(it != v.end() && *it == 8);
    it = std::partition_point(v.begin(), v.end(), less_n<10>());
    ccwrap_test(it == v.end());

    v3 = std::move(v2);
    ccwrap_test(v3.size() == 2 && v3[0] == 3 && v3[1] == 1);
    ...
    ccwrap_test(sv[2] == "zero" && sv[3] == "one" && sv[4] == "two");
...
CCWRAP_TEST(functional) {
    ...
    SubClass1   sc1;
    ccwrap_test(std::mem_fn(&SubClass1::mfn1)(sc1, 1) == 2);
    ccwrap_test(std::mem_fn(&SubClass1::mfn2)(sc1, 1, 2) == 3);

    typedef unsigned char uc;
    ccwrap_test(std::bit_and<uc>()(uc(0xcc),uc(0x88)) == uc(0xcc & 0x88));
    ccwrap_test(std::bit_or<uc>()(uc(0x50),uc(0x0c)) == uc(0x50 | 0x0c));
    ccwrap_test(std::bit_xor<uc>()(uc(0x0f),uc(0xff)) == uc(0x0f ^ 0xff));
    //ccwrap_test(std::bit_not<uc>()(uc(0x33)) == ~0x33);
...
void atomic_test1() {
    ...
    ccwrap_test(before == 1);
    ccwrap_test(ai.load() == 3);
    before = ai.fetch_sub(1);
    ccwrap_test(before == 3);
    ccwrap_test(ai.load() == 2);
    before = ai.fetch_or(1);
    ccwrap_test(before == 2);
    ccwrap_test(ai.load() == 3);
    before = ai.fetch_and(1);
    ccwrap_test(before == 3);
    ccwrap_test(ai.load() == 1);
    ...
    ccwrap_test(ai.load() == 2);
    ai += 3;
    ccwrap_test(ai.load() == 5);
    ai -= 4;
    ccwrap_test(ai.load() == 1);
    ai |= 6;
    ccwrap_test(ai.load() == 7);
    ai &= 5;
    ccwrap_test(ai.load() == 5);
    ai ^= 1;
    ccwrap_test(ai.load() == 4);
}
...
#endif  // __CCWRAP_NO_HEADER_ATOMIC
...
CCWRAP_TEST(etc) {
    ...
    ccwrap_test(!p.empty());
    std::any    a1;
    std::variant<int, float, std::string>   va1;
    std::optional<int>      opi1;
    std::tuple<int, char, float>    tu1;
