...
TEST_SUITE(boost2std) {
	...
	TEST_CASE(stl_test, algorithm) {
	    std::vector<int>    v;
	    v.push_back(3);
	    v.push_back(1);
	    v.push_back(8);

	    test_true( std::all_of(v.begin(), v.end(), less_n<10>()) );
	    test_true( std::all_of(v.begin(), v.end(), less_n<5>()) == false);

	    test_true( std::any_of(v.begin(), v.end(), less_n<5>()) );
	    test_true( std::any_of(v.begin(), v.end(), less_n<0>()) == false );

	    std::vector<int>    v2(3);
	    std::copy_if(v.begin(), v.end(), v2.begin(), less_n<5>());
	    test_true(v2[0] == 3 && v2[1] == 1 && v2[2] == 0);

	    std::copy_n(v.begin(), 3, v2.begin());
	    test_true(v2[0] == 3 && v2[1] == 1 && v2[2] == 8);

	    test_true(std::find_if_not(v.begin(), v.end(), less_n<10>()) == v.end());
	    test_true(std::find_if_not(v.begin(), v.end(), less_n<0>()) == v.begin());

	    v2.clear();
	    std::vector<int> v3;
	    std::partition_copy(v.begin(), v.end(), std::back_inserter(v2), std::back_inserter(v3), less_n<5>());
	    test_true(v2.size() == 2 && v2[0] == 3 && v2[1] == 1 && v3.size() == 1 && v3[0] == 8);

	    std::vector<int>::iterator  it = std::partition_point(v.begin(), v.end(), less_n<5>());
	    test_true(it != v.end() && *it == 8);
	    it = std::partition_point(v.begin(), v.end(), less_n<10>());
	    test_true(it == v.end());

	    v3 = std::move(v2);
	    test_true(v3.size() == 2 && v3[0] == 3 && v3[1] == 1);
	    ...
	    test_true(sv[2] == "zero" && sv[3] == "one" && sv[4] == "two");
	...
	TEST_CASE(stl_test, functional) {
	    ...
	    SubClass1   sc1;
	    test_true(std::mem_fn(&SubClass1::mfn1)(sc1, 1) == 2);
	    test_true(std::mem_fn(&SubClass1::mfn2)(sc1, 1, 2) == 3);

	    typedef unsigned char uc;
	    test_true(std::bit_and<uc>()(uc(0xcc),uc(0x88)) == uc(0xcc & 0x88));
	    test_true(std::bit_or<uc>()(uc(0x50),uc(0x0c)) == uc(0x50 | 0x0c));
	    test_true(std::bit_xor<uc>()(uc(0x0f),uc(0xff)) == uc(0x0f ^ 0xff));
	    //test_true(std::bit_not<uc>()(uc(0x33)) == ~0x33);
