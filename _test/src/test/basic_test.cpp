...
TEST_SUITE(basic) {
	...
	TEST_CASE(basic_test, check) {
	    ...
	    test_eq(ao2, 1);
	    test_eq(ao3, 4);
	    test_eq(ao4, sizeof(void*));
	    test_eq(alignof(std::uint64_t), 8);

	  #ifndef __CCWRAP_NO_THREAD_LOCAL
	    th_int_val = 50;
	    test_eq(th_int_val, 50);
	  #endif
	 ...
	 #if __CCWRAP__ > 0
	    //test_true(strcasecmp(p, "HELLO") == 0);
	 #else
	    test_true(strcmp(p, "hello") == 0);
	#endif
	    delete[] p;
	    ...
	    test_true(std::strcmp(buf, "# 1234567890123456789  123456789abcdef0\n") == 0);
	 #endif

	 #ifndef __CCWRAP_NO_ALIGNAS
	    test_eq(uintptr_t(&align8_int_val1) & 7, 0);
	    test_eq(uintptr_t(&align8_int_val2) & 7, 0);
	 #endif
	}
