 ...
 */
#ifndef _CCW_DETAIL_STRING_VIEW_HPP
#define _CCW_DETAIL_STRING_VIEW_HPP
...
public:
 ...
 #if __cplusplus >= 201103L //ndef __CCW_NO_STATIC_CONST_INT_MEMBER_INITIALIZATION
    static constexpr size_type                      npos = size_type(-1);
 #elif __CCW_NO_STATIC_CONST_INT_MEMBER_INITIALIZATION
...
public:
    ...
    // C++20 prefix/suffix checks
    bool starts_with(basic_string_view sv) const { return size_ >= sv.size_ && compare(0, sv.size_, sv) == 0; }
    bool starts_with(value_type c)         const { return size_ != 0 && ptr_[0] == c; }
    bool starts_with(const value_type* s)  const { return starts_with(basic_string_view(s)); }
    bool ends_with(basic_string_view sv)   const { return size_ >= sv.size_ && compare(size_ - sv.size_, sv.size_, sv) == 0; }
    bool ends_with(value_type c)           const { return size_ != 0 && ptr_[size_ - 1] == c; }
    bool ends_with(const value_type* s)    const { return ends_with(basic_string_view(s)); }
...
#if __cplusplus < 201103L //ndef __CCW_NO_STATIC_CONST_INT_MEMBER_INITIALIZATION
#ifdef __CCW_NO_STATIC_CONST_INT_MEMBER_INITIALIZATION
template< typename C, class T >
...
void basic_string_view<C,T>::throw_out_of_range() {
    _CCW_THROW(std::out_of_range("std::basic_string_view"));
}
...
#undef  _CCW_CS
#undef  _CCW_SV_OP
#define _CCW_SV     const basic_string_view<C,T>&
#define _CCW_CS     const C*
#define _CCW_SV_OP(_op,_op2)                       \
template<typename C, class T>                                   \
inline bool operator _op(_CCW_SV lhs, _CCW_SV rhs) {    \
    return lhs.compare(rhs) _op 0;                              \
}                                                               \
template<typename C, class T>                                   \
inline bool operator _op(_CCW_SV lhs, _CCW_CS rhs) {    \
    return lhs.compare(rhs) _op 0;                              \
}                                                               \
template<typename C, class T>                                   \
inline bool operator _op(_CCW_CS lhs, _CCW_SV rhs) {    \
...
// std::hash<basic_string_view<C,T>> (FNV-1a over the viewed characters).
template<class> struct hash;   // primary (also declared by detail/hashtable.hpp)
template<class C, class T> struct hash<basic_string_view<C, T> > {
    std::size_t operator()(const basic_string_view<C, T>& sv) const {
        std::size_t h = std::size_t(2166136261u);
        const C* p = sv.data();
        for (std::size_t i = 0; i < sv.size(); ++i) { h ^= std::size_t((unsigned)p[i]); h *= std::size_t(16777619u); }
        return h;
    }
};
...
namespace _ccw {
    using _CCW_STD17::basic_string_view;  using _CCW_STD17::string_view;  using _CCW_STD17::wstring_view;
    using _CCW_STD17::u16string_view;     using _CCW_STD17::u32string_view;
#if defined(__cpp_char8_t) || defined(__CCW_HAS_CHAR8_T)
    using _CCW_STD17::u8string_view;
#endif
}
