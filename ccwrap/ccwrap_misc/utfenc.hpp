...
namespace __impl {
	...
	std::size_t count_chars_impl(SC const* bgn, SC const* end, bool* er_p=NULL, char32 altc=UTFENC_CVT_ALT_CH) {
	...
	template<typename DC, typename SC> inline
	std::size_t count_chars(SC const* bgn, SC const* end, bool* er_p=NULL, char32 altc=UTFENC_CVT_ALT_CH) {
		typedef typename to_uchar<SC>::type SUC;
		return count_chars_impl<DC>((SUC const*)bgn, (SUC const*)end, er_p, altc);
	}
...
bool convert(DST& dst, SRC const& src, char32 altc=UTFENC_CVT_ALT_CH, typename SRC::value_type const* = 0) {
	typedef typename to_uchar<typename SRC::value_type>::type SUC;
