 ...
 *  @note
 */
#ifndef __CCWRAP_FILESYSTEM_PATH_HPP_INCLUDED
#define __CCWRAP_FILESYSTEM_PATH_HPP_INCLUDED
...
#include "../ccwrap_misc/str_enc.hpp"
#include "../ccwrap_misc/strpath.hpp"
...
namespace _ccwrap {
	...
	private:
	    ...
	    template <class D, class S> static void conv(D& dst, S& src) {
			ccwrap::str_enc::convert(dst, src);
			//dst = ccwrap::stringCodeConv<D>(src);
		}

	    template <class D, class S> static void conv_add(D& dst, S& src) {
			dst += ccwrap::str_enc::to<D>(src);
			//ccwrap::str_enc::convert_add(dst, src);
			//dst += ccwrap::stringCodeConv<D>(src);
		}
...
template<class L, class C> inline bool 
operator==(L const& l, _ccwrap::basic_path<C> const& r) noexcept { return r.operator==(l); }
template<class L, class C> inline bool 
operator< (L const& l, _ccwrap::basic_path<C> const& r) noexcept { return r.operator> (l); }
template<class L, class C> inline bool 
operator> (L const& l, _ccwrap::basic_path<C> const& r) noexcept { return r.operator< (l); }
...
#endif	// __CCWRAP_FILESYSTEM_PATH_HPP_INCLUDED
